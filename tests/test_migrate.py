...
class TestMigrateProject:
    ...
    def test_migrate_project_success(self, tmp_path):
        ...
        with patch(
            "fascraft.commands.migrate.analyze_current_structure"
        ) as mock_analyze:
            with patch("fascraft.commands.migrate.confirm_migration") as mock_confirm:
                with patch("fascraft.commands.migrate.create_backup") as mock_backup:
                    with patch(
                        "fascraft.commands.migrate.create_base_router_structure"
                    ) as mock_base:
                        with patch(
                            "fascraft.commands.migrate.migrate_to_domain_modules"
                        ) as mock_migrate:
                            with patch(
                                "fascraft.commands.migrate.update_main_py_for_migration"
                            ) as mock_update:
                                with patch(
                                    "fascraft.commands.migrate.create_fascraft_config"
                                ) as mock_config:
                                    with patch(
                                        "fascraft.commands.migrate.display_migration_summary"
                                    ) as mock_summary:
