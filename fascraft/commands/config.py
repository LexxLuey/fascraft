...
def manage_config(
    ...
    path: str = typer.Option(
        ".", help="📁 The path to the FastAPI project"
    ),
    ...
    if action == "show":
        ...
        if not key or not value:
            error_text = Text(
                "❌ Error: Both --key and --value are required for update action.", style="bold red"
            )
            console.print(error_text)
            raise typer.Exit(code=1)
        ...
        error_text = Text(
            f"❌ Error: Unknown action '{action}'. Use: show, create, update, or validate", style="bold red"
        )
        console.print(error_text)
        raise typer.Exit(code=1)
...
def show_config(config_path: Path, project_path: Path) -> None:
    ...
    try:
        ...
        console.print(f"📋 FasCraft Configuration for: {project_path.name}", style="bold green")
        ...
        error_text = Text(
            f"❌ Error reading configuration: {str(e)}", style="bold red"
        )
        console.print(error_text)
        raise typer.Exit(code=1)
...
def create_config(config_path: Path, project_path: Path) -> None:
    ...
    try:
        ...
        console.print("✅ FasCraft configuration created successfully!", style="bold green")
...
def update_config(config_path: Path, key: str, value: str) -> None:
    ...
    if not config_path.exists():
        console.print("❌ No FasCraft configuration found.", style="bold red")
        console.print("💡 Use 'fascraft config create' to create one first.", style="yellow")
        return
    ...
    try:
        ...
        if len(keys) != 2:
            error_text = Text(
                "❌ Error: Key must be in format 'section.key' (e.g., 'project.name')", style="bold red"
            )
            console.print(error_text)
            raise typer.Exit(code=1)
        ...
        if section not in config:
            error_text = Text(
                f"❌ Error: Section '{section}' not found in configuration", style="bold red"
            )
            console.print(error_text)
            raise typer.Exit(code=1)
    ...
    try:
        ...
        if "project" in config:
            ...
            if "name" in project and project["name"]:
                validation_table.add_row("project.name", "✅ Valid", f"'{project['name']}'")
            else:
                validation_table.add_row("project.name", "❌ Invalid", "Must not be empty")
        
        # Validate router section
        if "router" in config:
            router = config["router"]
            if "base_prefix" in router and router["base_prefix"].startswith("/"):
                validation_table.add_row("router.base_prefix", "✅ Valid", router["base_prefix"])
            else:
                validation_table.add_row("router.base_prefix", "❌ Invalid", "Must start with '/'")
        ...
        if "database" in config:
            ...
            if "default" in db and db["default"] not in db.get("supported", []):
                warnings.append(f"Default database '{db['default']}' not in supported list")
